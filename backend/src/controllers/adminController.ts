...
export const crearUsuario = async (req: AuthRequest, res: Response) => {
  try {
    ...
    const result = await pool.query(
      `INSERT INTO usuarios (nombre_completo, apellido, email, contraseña_hash, rol)
       VALUES ($1, $2, $3, $4, $5) RETURNING id, nombre_completo, email, rol`,
      [nombre_completo, apellido || '', email, contrasena_hash, rol]
    );
...
export const editarUsuario = async (req: AuthRequest, res: Response) => {
  try {
    ...
    let params: any[] = [nombre_completo, apellido, email, rol];
...
export const crearAsignatura = async (req: AuthRequest, res: Response) => {
  try {
    const { nombre, codigo, creditos, semestre } = req.body;

    const result = await pool.query(
      `INSERT INTO asignaturas (nombre, codigo, creditos, semestre)
       VALUES ($1, $2, $3, $4) RETURNING id, nombre, codigo, creditos, semestre`,
      [nombre, codigo || '', creditos || 3, semestre || 1]
    );
...
export const editarAsignatura = async (req: AuthRequest, res: Response) => {
  try {
    const { id } = req.params;
    const { nombre, codigo, creditos, semestre } = req.body;

    await pool.query(
      `UPDATE asignaturas SET nombre = $1, codigo = $2, creditos = $3, semestre = $4 WHERE id = $5`,
      [nombre, codigo, creditos, semestre, id]
    );
...
export const eliminarAsignatura = async (req: AuthRequest, res: Response) => {
  try {
    const { id } = req.params;
...
export const relacionarDocenteAsignatura = async (req: AuthRequest, res: Response) => {
  try {
    const { docente_id, asignatura_id } = req.body;

    await pool.query(
      `INSERT INTO docente_asignatura (usuario_id, asignatura_id, activo)
       VALUES ($1, $2, true)`,
...
export const obtenerDocentesPorAsignatura = async (req: AuthRequest, res: Response) => {
  try {
    const { asignaturaId } = req.params;
    const result = await pool.query(
      `SELECT u.id, u.nombre_completo, u.email FROM usuarios u
       JOIN docente_asignatura da ON u.id = da.usuario_id
       WHERE da.asignatura_id = $1 AND u.rol = 'docente'`,
      [asignaturaId]
    );
    res.json(result.rows);
  } catch (e: any) {
...
export const asignarEstudianteAsignatura = async (req: AuthRequest, res: Response) => {
  try {
...
export const obtenerEstudiantesPorAsignatura = async (req: AuthRequest, res: Response) => {
  try {
    const { asignaturaId } = req.params;
    const result = await pool.query(
      `SELECT u.id, u.nombre_completo, u.email FROM usuarios u
       JOIN estudiante_asignatura ea ON u.id = ea.estudiante_id
       WHERE ea.asignatura_id = $1 AND u.rol = 'estudiante'`,
      [asignaturaId]
    );
    res.json(result.rows);
  } catch (e: any) {
